...
def gfg():
    ...
    if request.method == "POST":
        ...
        nitrogen = request.form.get("Nitrogen Content of Soil",type=int)
        phosphorus = request.form.get("Phosphorus Content of Soil",type=int)
        potassium = request.form.get('Potassium Content of Soil',type=int)
        temperature = request.form.get('Temperature of Environment (C)',type=int)
        humidity = request.form.get('Humidity of Environment (%)',type=int)
        ph = request.form.get('pH of Soil',type=int)
        rainfall = request.form.get('Annual Rainfall (mm)',type=int)
        ...
        for i in range(22):
            if results.loc[i, "score"] > 0.001:
                temp.update({str(labels[results.loc[i, "index"]]) : str(round(100 * results.loc[i, "score"]))})
                print("- " + labels[results.loc[i, "index"]] + " (" + str(round(100 * results.loc[i, "score"])) + "%)")
               
        
    return render_template("form.html",temp=temp)
